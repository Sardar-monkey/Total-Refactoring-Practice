public class SalesReportGenerator {
    public String generate(Date from, Date to) {
        ...
        List<Sale> data = db.query("SELECT * FROM sales WHERE date BETWEEN ? AND ?", from, to);

        if (data.isEmpty())
        { 
            sb.append("No data"); return sb.toString(); 
        }
...
public class InventoryReportGenerator {
    public String generate(Date from, Date to) {
        ...
        List<Item> data = db.query("SELECT * FROM inventory WHERE updated BETWEEN ? AND ?", from, to);
